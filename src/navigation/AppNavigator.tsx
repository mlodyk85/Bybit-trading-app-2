...
export const AppNavigator: React.FC = () => {
  ...
  const [smartSeeds, setSmartSeeds] = useState<Record<string, AssetSmartAutoSeed>>({});
  const [maxOrderUsdt, setMaxOrderUsdt] = useState(MAX_SPOT_ORDER_USDT);
  const { credentials, account, positions, connectionState, errorMessage, isLoading, isRefreshing, lastRefreshTime, autoRefreshEnabled, autoRefreshInterval, setAutoRefreshEnabled, setAutoRefreshInterval, connect, testConnection, disconnect, refresh } = useBybitAccount();

  useEffect(() => { let mounted = true; void loadMaxOrderUsdt().then((value) => { if (mounted) setMaxOrderUsdt(value); }); return () => { mounted = false; }; }, []);
  const updateMaxOrder = async (value: number) => { const saved = await saveMaxOrderUsdt(value); setMaxOrderUsdt(saved); };

  if (isLoading && !credentials) return <View style={styles.loadingContainer}><ActivityIndicator size="large" color="#F0B90B" /><Text style={styles.loadingText}>Ładowanie aplikacji...</Text></View>;
  if (!credentials || connectionState === 'disconnected') return <SetupScreen onConnect={connect} onTest={testConnection} initialApiKey={credentials?.apiKey || ''} initialApiSecret={credentials?.apiSecret || ''} />;

  return <NavigationContainer theme={{ dark: true, colors: { primary: '#F0B90B', background: '#121212', card: '#1E1E1E', text: '#FFFFFF', border: '#2C2C2C', notification: '#F0B90B' } }}>
    <Tab.Navigator screenOptions={{ headerShown: false, tabBarStyle: { backgroundColor: '#1E1E1E', borderTopColor: '#2C2C2C', height: 62, paddingBottom: 7, paddingTop: 6 }, tabBarActiveTintColor: '#F0B90B', tabBarInactiveTintColor: '#8E8E93', tabBarLabelStyle: { fontSize: 9, fontWeight: '600' } }}>
      <Tab.Screen name="Portfolio" options={{ tabBarLabel: 'Portfel', tabBarIcon: ({ color }) => <Text style={{ color, fontSize: 17 }}>📊</Text> }}>
        {({ navigation }) => <PortfolioScreen credentials={credentials} account={account} connectionState={connectionState} errorMessage={errorMessage} isRefreshing={isRefreshing} lastRefreshTime={lastRefreshTime} autoRefreshEnabled={autoRefreshEnabled} onRefresh={refresh} onToggleAutoRefresh={setAutoRefreshEnabled} onOpenSmartAuto={(seed) => { setSelectedTradeSymbol(seed.symbol); setSmartSeed(seed); setSmartSeeds((current) => ({ ...current, [seed.symbol]: seed })); navigation.navigate('Trade'); }} />}
      </Tab.Screen>
      <Tab.Screen name="Positions" options={{ tabBarLabel: 'Pozycje', tabBarIcon: ({ color }) => <Text style={{ color, fontSize: 17 }}>📈</Text> }}>{() => <PositionsScreen credentials={credentials} positions={positions} account={account} isRefreshing={isRefreshing} onRefresh={refresh} />}</Tab.Screen>
      <Tab.Screen name="Trade" options={{ tabBarLabel: 'Trade', tabBarIcon: ({ color }) => <Text style={{ color, fontSize: 17 }}>⚡</Text> }}>{() => <TradeScreen credentials={credentials} initialSymbol={selectedTradeSymbol} initialHolding={smartSeed} initialHoldings={Object.values(smartSeeds)} maxOrderUsdt={maxOrderUsdt} onHoldingConsumed={() => { setSmartSeed(null); setSelectedTradeSymbol(''); }} />}</Tab.Screen>
