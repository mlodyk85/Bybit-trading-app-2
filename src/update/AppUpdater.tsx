  ...
  useEffect(() => {
    ...
    const check = async () => {
      try {
        ...
        if (!asset || !isNewerVersion(release.tag_name, currentVersion)) return;

        Alert.alert(
          'Dostępna aktualizacja',
          `Nowa wersja ${release.tag_name} jest gotowa. Aplikacja może pobrać ją automatycznie. Android poprosi tylko o potwierdzenie instalacji.`,
          [
            { text: 'Później', style: 'cancel' },
            {
              text: 'Aktualizuj',
              onPress: () => void downloadAndInstall(asset.browser_download_url, release.tag_name),
            },
          ]
        );
      } catch {
    ...
    <View style={styles.overlay} pointerEvents="box-none">
      <View style={styles.card}>
        ...
        <TouchableOpacity disabled style={styles.button}>
          <Text style={styles.buttonText}>Aktualizacja w toku</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
