...
function formatUsdt(value: number): string {
  ...
  return `${value.toLocaleString('pl-PL', { minimumFractionDigits: 2, maximumFractionDigits: 4 })} USDT`;
}
      ...
      {tradable && (
        <View style={styles.tradeBox}>
          ...
          <View style={styles.tradeLine}>
            ...
            <Text style={styles.currentValue}>{Number.isFinite(currentValue) ? formatUsdt(currentValue) : '-'}</Text>
          </View>
          {Number.isFinite(deltaFromLastBuy) && (
            <View style={styles.tradeLine}>
              <Text style={styles.tradeLabel}>Różnica vs ostatni BUY</Text>
              <Text style={[styles.currentValue, deltaFromLastBuy >= 0 ? styles.positive : styles.negative]}>
                {deltaFromLastBuy >= 0 ? '+' : ''}{formatUsdt(deltaFromLastBuy)}
              </Text>
            </View>
          )}
...
const styles = StyleSheet.create({
  ...
  detailCol: { flex: 1 },
  detailColRight: { flex: 1, alignItems: 'flex-end' },
  detailLabel: { color: '#8E8E93', fontSize: 11, marginBottom: 2 },
  detailValue: { color: '#E0E0E0', fontSize: 13, fontWeight: '500' },
  tradeBox: { marginTop: 12, paddingTop: 10, borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: '#333333' },
  tradeLine: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 5 },
  tradeLabel: { color: '#8E8E93', fontSize: 11 },
  buyValue: { color: '#22C55E', fontSize: 12, fontWeight: '700' },
  sellValue: { color: '#F59E0B', fontSize: 12, fontWeight: '700' },
  currentValue: { color: '#D4D4D8', fontSize: 12, fontWeight: '700' },
  positive: { color: '#22C55E' },
  negative: { color: '#EF4444' },
  tradeNote: { color: '#66666B', fontSize: 10, marginTop: 8, lineHeight: 14 },
});
