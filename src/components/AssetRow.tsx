...
interface AssetRowProps { asset: CoinBalance; executions?: SpotExecution[]; onOpenTrade?: (seed: AssetSmartAutoSeed) => void; }
...
export const AssetRow: React.FC<AssetRowProps> = ({ asset, executions = [], onOpenTrade }) => {
  ...
  return <TouchableOpacity activeOpacity={canOpenSmart ? 0.78 : 1} disabled={!canOpenSmart} onPress={() => canOpenSmart && onOpenTrade?.({ symbol, baseQty: managedQty, buyPrice, buyCostUsdt })} style={styles.container}>
    <View style={styles.coinHeader}><View><Text style={styles.coinName}>{asset.coin}</Text>{canOpenSmart && <Text style={styles.openHint}>SMART ACCUMULATE {coin} →</Text>}</View><Text style={styles.usdValue}>{formatCurrency(asset.usdValue, 'USD')}</Text></View>
